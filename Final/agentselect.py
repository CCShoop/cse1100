...
def select_agents(select_win, playersCheckedVars):
    ...
    tk.Button(picked_win, text='Copy', bg='gray20', fg='white', relief='ridge', command=lambda: copy(picked_win, text)).pack(pady=5)
    ttk.Separator(picked_win, orient='horizontal').pack(fill='x', pady=5)
    # Closes this window
    tk.Button(picked_win, text='Close', bg='gray20', fg='white', relief='ridge', command=lambda: picked_win.destroy()).pack(pady=5)
...
def add_agent(window):
    ...
    add_a_win.configure(bg='black')
    add_a_win.title('Add Agent(s) to Player')
    tk.Label(add_a_win, bg='black', fg='white', text='Select a Player:').pack(padx=75, pady=5)
...
def show_agents(add_win, player, menu_button):
    ...
    for agent in agentConfig.sections(): # Displays a checkbox for each player
        tk.Checkbutton(add_win, bg='black', activebackground='gray20', fg='red', activeforeground='white', text=agent, variable=agentsCheckedVars[counter][1], onvalue=1, offvalue=0).pack()
        counter += 1
    # Saves player name and agents to config file
    tk.Button(add_win, text='Save', bg='gray20', fg='white', relief='ridge', command=lambda: a_config(add_win, player, agentsCheckedVars)).pack(pady=5)
...
def add_player(window):
    ...
    tk.Label(add_win, bg='black', fg='white', text='Player Name:').pack(pady=5) # Label for input field
    ...
    if agentConfig.sections():
        ttk.Separator(add_win, orient='horizontal').pack(fill='x', pady=5)
        tk.Label(add_win, bg='black', fg='white', text='Select this Player\'s Unlocked Agents:').pack(pady=5)
        ...
        counter = 0
        for agent in agentConfig.sections(): # Displays a checkbox for each player
            tk.Checkbutton(add_win, bg='black', activebackground='gray20', fg='red', activeforeground='white', text=agent, variable=agentsCheckedVars[counter], onvalue=1, offvalue=0).pack()
            counter += 1
    # Saves player name and agents to config file
    tk.Button(add_win, text='Save', bg='gray20', fg='white', relief='ridge', command=lambda: p_config(add_win, intake, agentsCheckedVars)).pack(pady=5)
    ttk.Separator(add_win, orient='horizontal').pack(fill='x', pady=5)
    tk.Button(add_win, text='Back', bg='gray20', fg='white', relief='ridge', command=lambda: add_win.destroy()).pack(pady=5)
...
def new_agent(window):
    ...
    tk.Label(new_win, bg='black', fg='white', text='Agent Name:').pack(pady=5) # Label for input field
    entryName = tk.StringVar() # For taking player's name as input
    intake = tk.Entry(new_win, textvariable=entryName, width=40)
    intake.pack(padx=10, pady=5) # Text entry field
    tk.Button(new_win, text='Save', bg='gray20', fg='white', relief='ridge', command=lambda: a_n_config(new_win, intake)).pack(pady=5)
    ttk.Separator(new_win, orient='horizontal').pack(fill='x', pady=5)
    tk.Button(new_win, text='Back', bg='gray20', fg='white', relief='ridge', command=lambda: new_win.destroy()).pack(pady=5)
...
def new_map(window):
    ...
    tk.Label(new_win, bg='black', fg='white', text='Map Name:').pack(pady=5) # Label for input field
    entryName = tk.StringVar() # For taking player's name as input
    intake = tk.Entry(new_win, textvariable=entryName, width=40)
    intake.pack(padx=10, pady=5) # Text entry field
    tk.Button(new_win, text='Save', bg='gray20', fg='white', relief='ridge', command=lambda: m_config(new_win, intake)).pack(pady=5)
    ttk.Separator(new_win, orient='horizontal').pack(fill='x', pady=5)
    tk.Button(new_win, text='Back', bg='gray20', fg='white', relief='ridge', command=lambda: new_win.destroy()).pack(pady=5)
...
def main():
    ...
    for agent in agentConfig.sections():
        agents.append(agent)
    agents.sort()
    newAConfig = cp.ConfigParser()
    for agent in agents:
        newAConfig.add_section(agent) # Sort all agents into new config file and write it
    with open('agents.ini', 'w') as configfile:
        newAConfig.write(configfile)
    global playerConfig
    ...
    for player in playerConfig.sections():
        for option in playerConfig.options(player):        # Remove all agent options
            if not playerConfig.getint(player, option):    # with a value of 0
    ...
    sortingList.sort()
    newPConfig = cp.ConfigParser()      # Same as agents sorting,
    for item in sortingList:            # but also has options
        newPConfig.add_section(item[1]) # with unlocked agents
        item[2].sort()
        for option in item[2]:
            newPConfig.set(item[1], str(option), '1')
    with open('players.ini', 'w') as configfile: # It actually sorts it by number of
        newPConfig.write(configfile)             # unlocked agents to make selection
    global mapConfig                             # more successful
    ...
    if playerConfig.sections() and agentConfig.sections():
        # Buttons
        tk.Label(window, bg='black', fg='white', text='Select current players:').pack(padx=xPadding, pady=5)
        playersCheckedVars = []
        ...
        for player in playerConfig.sections(): # Displays a checkbox for each player
            tk.Checkbutton(window, bg='black', activebackground='gray20', fg='red', activeforeground='white', text=player, variable=playersCheckedVars[counter], onvalue=1, offvalue=0).pack()
            counter += 1
        tk.Button(window, text='Select Agents', bg='gray20', fg='white', relief='ridge', command=lambda: select_agents(window, playersCheckedVars)).pack(padx=50, pady=5)
        ttk.Separator(window, orient='horizontal').pack(fill='x', pady=5)
    if mapConfig.sections():
        tk.Button(window, text='Select Map', bg='gray20', fg='white', relief='ridge', command=lambda: select_map(window)).pack(padx=50, pady=5)
        ttk.Separator(window, orient='horizontal').pack(fill='x', pady=5)
    if playerConfig.sections() and agentConfig.sections():
        tk.Button(window, text='Add Agent to Player', bg='gray20', fg='white', relief='ridge', command=lambda: add_agent(window)).pack(padx=xPadding, pady=5)
        ttk.Separator(window, orient='horizontal').pack(fill='x', pady=5)
    tk.Button(window, text='Add New Player', bg='gray20', fg='white', relief='ridge', command=lambda: add_player(window)).pack(padx=xPadding, pady=5)
    ttk.Separator(window, orient='horizontal').pack(fill='x', pady=5)
    tk.Button(window, text='Add New Agent', bg='gray20', fg='white', relief='ridge', command=lambda: new_agent(window)).pack(padx=xPadding, pady=5)
    ttk.Separator(window, orient='horizontal').pack(fill='x', pady=5)
    tk.Button(window, text='Add New Map', bg='gray20', fg='white', relief='ridge', command=lambda: new_map(window)).pack(padx=xPadding, pady=5)
    ttk.Separator(window, orient='horizontal').pack(fill='x', pady=5)
    tk.Button(window, text='Exit', bg='gray20', fg='white', relief='ridge', command=lambda: close(window)).pack(padx=xPadding, pady=5)
